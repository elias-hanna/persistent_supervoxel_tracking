...
// #include "../libs/papon/supervoxel/supervoxel_clustering.h"
// // #include "libs/sequential_supervoxel_clustering/sequential_supervoxel_clustering.h"
// // #include "libs/supervoxel_clustering/supervoxel_clustering.h"
// // #include "libs/viewer/viewer.hpp"
...
//   // Viewer Class to visualize pointcloud output from various capturing devices
//   // Viewer<PointT> viewer( *grabber );
//   // viewer.run();
...
//   //   if(!cloud.empty())
//   //   {
//   //     // super.setInputCloud((PointCloudT::Ptr)new PointCloudT(cloud));
...
typedef pcl::PointXYZL PointLT;
typedef pcl::PointCloud<PointLT> PointLCloudT;
typedef pcl_papon::Supervoxel::CentroidT SVCentroidT;
...
main (int argc, char ** argv)
{
  ...
  bool no_prune_seeds = pcl::console::find_switch (argc, argv, "--nopruneseeds");
  ...
  pcl_papon::SupervoxelClustering<PointT> super (voxel_resolution, seed_resolution,!disable_transform,!no_prune_seeds);
  ...
  std::map <uint32_t, pcl_papon::Supervoxel::Ptr > supervoxel_clusters;
  ...
  std::map <uint32_t, pcl_papon::Supervoxel::Ptr > refined_supervoxel_clusters;
  ...
  while (!viewer->wasStopped ())
  {
    if (show_supervoxels)
    {
      ...
      // }
      if (!viewer->updatePointCloud ((show_refined)?refined_voxel_labeled_cloud:voxel_labeled_cloud, "labeled voxels"))
      {
        viewer->addPointCloud ((show_refined)?refined_voxel_labeled_cloud:voxel_labeled_cloud, "labeled voxels");
    ...
    if (show_normals)
    {
      std::map <uint32_t, pcl_papon::Supervoxel::Ptr>::iterator sv_itr,sv_itr_end;
    ...
    if (show_graph && !graph_added)
    {
      ...
      for ( ; label_itr != label_adjacency.end (); )
      {
        ...
        pcl_papon::Supervoxel::Ptr supervoxel = supervoxel_clusters.at (supervoxel_label);
        //Now we need to iterate through the adjacent supervoxels and make a point cloud of them
        pcl::PointCloud<pcl_papon::Supervoxel::CentroidT> adjacent_supervoxel_centers;
        ...
        {     
          pcl_papon::Supervoxel::Ptr neighbor_supervoxel = supervoxel_clusters.at (adjacent_itr->second);
